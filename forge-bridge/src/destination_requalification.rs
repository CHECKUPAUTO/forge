...
//! module binds the destination evidence to the exact survivor provenance and
//! fails closed on mismatches or failed requalification.
...
pub struct DestinationRequalificationEvidenceV1 {
    ...
    /// Exact destination revision containing the gate and adapter.
    ...
    /// Content hash of the destination qualification contract or adapter.
...
/// has been independently requalified by the destination repository.
pub fn destination_promotion_permit(
    ...
    Ok(DestinationPromotionPermitV1 {
        destination_repository: evidence.destination_repository.clone(),
        destination_commit_id: evidence.destination_commit_id.clone(),
        candidate_id: evidence.candidate_id.clone(),
        search_evidence_id: evidence.search_evidence_id.clone(),
...
mod tests {
    ...
    #[test]
    fn exact_survivor_and_destination_evidence_mint_permit() {
        let permit = destination_promotion_permit(
            &provenance(ScientificSearchDispositionV1::Survivor),
            &evidence(),
        )
        .expect("matching destination qualification should permit promotion");
    ...
    fn destination_must_requalify_exact_search_survivor() {
        ...
        assert_eq!(
            destination_promotion_permit(&survivor, &wrong_candidate),
        ...
        assert_eq!(
            destination_promotion_permit(&survivor, &wrong_search),
    ...
    fn failed_destination_qualification_never_mints_permit() {
        ...
        assert_eq!(
            destination_promotion_permit(&survivor, &failed),
